...
```{r, plot of infectivity data}
summarydata$Dose <- plyr::mapvalues(summarydata$Dose, from = c("250","1000"), to = c("250 spores/mL", "1000 spores/mL"))

metschplottitle <- expression(paste("Infectivity of"~italic("Metschnikowia "),"spores stored at -20 and 4°C"))
...
metschplotplusboxplot <- summarydata %>%
  ...
  theme_bw()  + facet_wrap(Dose~storagetemp) +
...
save_plot("metschplotplusboxplot.jpg", metschplotplusboxplot, base_width = 10, base_height = 3)

summarydata$Dose <- plyr::mapvalues(summarydata$Dose, from = c("500","2000"), to = c("500 spores/mL", "2000 spores/mL"))

pastplottitle <- expression(paste("Infectivity of"~italic("Pasteuria "),"spores stored at -20 and 4°C"))
...
metschfridgedata <- summarydata %>%
  subset(summarydata$Parasite == "Metschnikowia" & summarydata$Fridge.or.freezer == "Fridge")
...
pastdata <- summarydata %>%
  subset(summarydata$Parasite == "Pasteuria")
...
sporeplottitle <- expression(paste("Spore yield from"~italic("Pasteuria- "),"and"~italic("Metschnikowia-"),"infected"~italic("D. dentifera "),"stored at -20 and 4°C"))
